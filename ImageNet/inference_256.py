...
from retriever_256 import DynamicReteiever
...
class Online_ICL:
    ...
    def get_response_ideficsv1(self,sample):
        ...
        if demonstrations is not None:
            for dm in demonstrations:
                ...
                prompt.append(f"Output:{dm.label}"+"\n")
    ...
    def evaluate_batch_on_idev1(self,batch_samples):
        ...
        for sample in batch_samples:
            demonstrations = self.retriever.get_demonstrations_from_bank(sample)
            prompt = []
            prompt.extend(["Instruction: provide an categpry to the image."+'\n'])
            if demonstrations is not None:
                for dm in demonstrations:
                    prompt.append(dm.image)
                    prompt.append(f"A photo of a:{dm.pseudo_label}"+"\n")
            prompt.append(sample.image)
            prompt.append(f"A photo of a:")
...
class FewShot:
    ...
    def evaluate_batch_on_idev1(self, batch_samples):
        ...
        for sample in batch_samples:
            demonstrations = self.retriever.get_demonstrations_from_bank(sample)
            demon = []
            demon.extend(["Instruction: Classify the following image into a category"+'\n'])

            for dm in demonstrations:
                demon.extend([
                    f"Image: {dm.image}",
                    f"Category:{dm.label.strip()}"+'\n'
                ])
            demon.extend([
                    f"Image:{sample.image}", # 添加图像
                    f"Category:"])
