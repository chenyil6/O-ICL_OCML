...
class DynamicReteiever:
    ...
    def update_based_on_new(self,query_sample):
        ...
        for d in self.dismatch_pool:
            ...
            # 靠近p
            d.feature_256_1024 = 0.8 * d.feature_256_1024 + 0.2 * p
        ...
        if query_sample.label == query_sample.pseudo_label:
            ...
            for m in self.match_pool:
                m.feature_256_1024  =0.8*m.feature_256_1024+ 0.2*query_sample.feature_256_1024
                updated_labels.add(m.label)

        # 更新已记录的每个类别标签的 prototype
        for label in updated_labels:
            # 获取当前类别的所有样本的 embed
            sample_features = [s.feature_256_1024 for s in self.label2sample[label]]
            # 计算新的 prototype 为该类别中所有样本的平均 embed
            self.label_to_prototype[label] = torch.mean(torch.stack(sample_features), dim=0)
