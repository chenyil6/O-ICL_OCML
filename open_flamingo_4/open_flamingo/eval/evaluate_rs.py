...
def evaluate_vqa(
    ...
    if control_signals["clip"]:
        ...
        # 读取json文件 ,存放的是 question -> answer_list
    with open(question_to_answerList_path, "r") as file:
        question2answer_list = json.load(file)

        # 读取json文件 ,存放的是 question_id -> answer_list
    with open(questionId_to_answerList_path, "r") as file:
            ...
            for i in range(len(batch["image"])):
                ...
                if control_signals["mismatch_type"] == "answer":
                    for ics in in_context_samples:
                        ...
                        answer_list_1 = question2answer_list[question]
                        answer_list_1 = list(set(answer_list_1))
                ...
                if control_signals["instruction_type"] == "instruction_1":
                    context_text = context_text+ "According to the previous question and answer pair, " \
                                   "answer the final question. "

                if control_signals["instruction_type"] == "instruction_2":
                    context_text = context_text + "According to the visual information in <image> token, " \
                                   "answer the following question. "

                if control_signals["instruction_type"] == "instruction_3":
                    context_text = context_text+ "Answer the question. "

                # Keep the text but remove the image tags for the zero-shot case
                if num_shots == 0:
                    context_text = context_text.replace("<image>", "")

                query_question =  batch["question"][i]

                # 对query question 做修改
                if control_signals["query_question_type"] == "del_W_N":
                    query_question = remove_words_by_pos(query_question, ['WP', 'WRB', 'NN'])

                if control_signals["query_question_type"] == "del_V":
                    query_question = remove_words_by_pos(query_question, ['VBZ'])

                if control_signals["query_question_type"] == "del_N":
                    query_question = remove_words_by_pos(query_question, ['NN'])

                if control_signals["query_question_type"] == "del_W":
                    query_question = remove_words_by_pos(query_question, ['WP', 'WRB'])

                query_text = eval_model.get_vqa_prompt(question = query_question)
